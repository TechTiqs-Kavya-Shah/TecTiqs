...
openai.api_key = "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"
        ...
        for subject in subjects:
            while True:
                study_time = questionary.text(
                    f"How much time do you want to study {subject} on {day}? (hh:mm): ").ask()
...
def input_start_time():
    while True:
        start_time = questionary.text("Enter start time for studying (hh:mm): ").ask()
...
def main():
    ...
    name = questionary.text("Hello, my name is daton. Please enter your name: ").ask()
    ...
    choice = questionary.select(
        ...
        choices=["Study Schedule Generator", "Notes Generator"]
    ).ask()
    ...
    if choice == "Study Schedule Generator":
        ...
        if not subjects_per_day:
            print(Fore.RED + "No subjects entered. Exiting program." + Style.RESET_ALL)
            return
        study_time_per_day = input_study_time(subjects_per_day)
        start_time = input_start_time()
        timetable = create_timetable(subjects_per_day, study_time_per_day, start_time)
        create_pdf_timetable(timetable)  # Create PDF timetable
        print(Fore.GREEN + "\nTimetable generated and saved as timetable.pdf" + Style.RESET_ALL)
        ...
        output_format = questionary.select(
            "How do you want to receive the notes?",
            choices=["Text", "Speech", "Image"]
        ).ask()
        ...
        want_videos = questionary.confirm("Do you want YouTube videos on the topic?").ask()
        if want_videos:
            youtube_link = generate_youtube_link(subject, chapter)
            print(f"Here is a YouTube link for {subject} Chapter {chapter}: {youtube_link}")
